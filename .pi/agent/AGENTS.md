...
- Live-Hosts sind von der Workstation aus grundsätzlich nur über den Jumphost erreichbar
  (`ssh database-jumphost01.live.ix01.sipgate.net`); direkter Zugriff von der Workstation aus
  wird per Design abgewiesen. „Connection refused“ von der Workstation zu einem Live-Host heißt
  also NICHT, dass ein Service fehlt oder ein Firewall-Eintrag fehlt — erst über den Jumphost
  (oder zentrale Logs) gegenprüfen, bevor ich irgendwas über Deploy-Zustand oder Paketfilter
  behaupte.
...
- **While a PR is under review, do not push every single fix** — accumulate changes
  ...
  place in the diff.
- **Politur ungepushter Commits SOFORT squaschen, nie als eigenen Commit stehen lassen**:
  Kommentar-Korrekturen, Umbenennungen, Sichtbarkeits-Nits, die beim Durchsehen eines
  ungepushten Commits entstehen — auch wenn der Fund NACH dem Commiten kommt —, wandern per
  `jj squash` direkt in den Commit, den sie polieren. Kein Fixup-Commit, keine Rückfrage, kein
  „squashe ich später": das Aufschreiben der Regel und das Zögern vor der Ausführung in
  derselben Antwort ist der Fehler, den die Regel meint.
...
- **„Warum funktioniert X nicht?“ ist eine Diagnose-Frage, kein Fix-Auftrag**:
Ursache recherchieren und erklären; den Fix nur vorschlagen. Anwenden —
insbesondere Änderungen an User-Config (Shell, Tools, Dotfiles) — erst auf
...
- **Transienter Zustand gehört in die Commit-Message, nicht in Code-Kommentare**: „noch nicht
  verifiziert", „bis X geheilt ist", „aktuell blockiert" verrotten still im Code, sobald der
  Zustand weg ist. Der Kommentar sagt nur dauerhafte Semantik; das Datum und der Vorbehalt
  leben im Commit, wo sie verrotten dürfen, ohne zu täuschen.
