...
- Error diagnoses are assertions too: for tool errors (404, denied, timeout) validate the
  cause first via a counter-test (e.g. try an alternative tool with different
  credentials) instead of presenting a plausible-sounding cause as the explanation.
- Scripted file edits (sed/perl/python str.replace) fail SILENTLY on pattern mismatch — after
  the edit, re-read the changed region and confirm it landed. Straalenabfang: beim Debuggen
...
- IntelliJ-vs-Maven-Buildzustand (Maven-Projekte ohne Delegation): beide teilen `target/`, aber
  ...
  Löschung, Maven-Sync und Reload überlebt. Inkrementelle Before-launch-Builds merken externe
  Wipes nicht (mvn clean) und kopieren Ressourcen nicht nach — nur *Build → Rebuild Project*
  resettet. IntelliJ importiert außerdem implizite Maven-Defaults nicht: `testResources` etc.
