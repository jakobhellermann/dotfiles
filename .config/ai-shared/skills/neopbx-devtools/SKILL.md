...
Session-Deep-Link (verbatim from user, verified format):
`https://neopbx-devtools.nautilus-tooling01.dev.ix01.sipgate.net/calls/<sessionId>`

Do NOT link the `RETEL_DEVTOOLS` URL from a `getCall` response's `links[]` as
"the devtools" — that is the retel devtools UI (raw ITI view), a different tool.
